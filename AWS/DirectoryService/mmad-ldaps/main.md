...
AWS Microsoft AD (以下AWS MMAD) と通信するときに ldaps が使えるとのことだったので、試してみました。
今回は、最速での ldaps 通信を目指します。
...
1. AWS MMAD ドメインコントローラーは、エンタープライズ CA と通信して、エンタープライズ CA が発行した証明書を取得します。
AWS MMAD ドメインコントローラーは、スタンドアロン CA および、サードパーティCA の証明書をサポートしていないので、**エンタープライズ CA からの証明書発行を行うことが必須となります。**
...
2. AD 管理サーバから ldp ツールを使用して、LDAPS 接続を行います。
AWS MMAD のドメインコントローラーにアタッチされた SG は、 LDAPSポート: 636 が開いているので、証明書を設置するだけで LDAPS による通信が可能です。

## ldaps 通信を有効化する手順

### エンタープライズ CA 用 Windows サーバ の構築

エンタープライズ CA 用 Windowsサーバを構築します。

OS: Windows2019 のサーバを一台、パブリックサブネットに構築してください。  
[こちら]() の Cloudformation テンプレートを流すと早いです。

### エンタープライズ CA 用 Windwos サーバのドメイン参加
...
ここからエンタープライズ CA として Windows サーバが機能するよう、セットアップを行います。
...
続いて、**AD CS** の設定を行って、ルートかつ、エンタープライズ CA の認証局をセットアップします。
...
### AWS MMAD ドメインコントローラーと エンタープライズ CA 間の通信許可
...
AD が エンタープライズ CA に、証明書を自動取得するまで、最大で30分ほどかかるので気長に待ちましょう。

### AWS MMAD との ldaps 通信の確認

最後に、LDAPS 通信が出来ているか AD 管理サーバから LDAPS 通信して確認します。

#### windows サーバからの疎通確認

AD 管理サーバに RDP ログインして、ldp ツール実行しましょう。
1. デスクトップで [ ldp ] で検索して、アプリケーションを開始します。
1. [ Connect ] を押して、"Server" に "ADドメイン名", Portに "636", SSL にチェックを付けて、[ OK ] を押します。

接続できると証明書情報が返ってくるはずです!
